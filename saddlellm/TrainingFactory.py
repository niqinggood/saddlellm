...
class LLMTrainingFactory:
    ...
    def blueprint(self) -> Dict:
        return {
            ...
            "artifacts": {
                ...
                "configs": "orchestrator configs and experiment manifests",
    ...
    def create_model_experiments(
        ...
        **overrides,
        ...
        from .ModelExperimentPlanner import ModelExperimentConfig, ModelExperimentPlanner
    ...
    def create_multimodal_plan(
        ...
        **overrides,
        ...
        from .VisionBackbones import list_vision_backbones
        ...
        recipe = TrainingRecipe(
            ...
            model=RecipeModelConfig(
                ...
                backend="saddle",
                params=blueprint.estimate_total_params(),
            ),
            ...
            multimodal=RecipeMultimodalConfig(
                enabled=True,
                data_path=data_path,
            ...
            logging=RecipeLoggingConfig(output_dir=output_dir, backend="local"),
        )
        compiled = recipe.compile(base_dir=output_dir, save_backend_plan=False)
        compiled["model_blueprint"] = blueprint.to_dict()
        payload = {
            "output_dir": output_dir,
            "blueprint": blueprint.to_dict(),
            "recipe": recipe.to_dict(),
            "orchestrator_config": compiled,
        ...
        if save:
            ...
            payload["recipe_path"] = recipe.save(os.path.join(output_dir, "mllm_sft_recipe.yaml"))
            payload["orchestrator_config_path"] = self._save_json(os.path.join(output_dir, "orchestrator_config.json"), compiled)
    ...
    def create_post_training_plan(
        ...
        **overrides,
        ...
        recipe = TrainingRecipe(
            ...
            experiment=overrides.pop("experiment", f"{self.config.domain}-{stage}-{preference_method if stage == 'preference' else method}"),
            stage=[stage, "eval"],
            model=RecipeModelConfig(
                name_or_path=overrides.pop("base_model", self.config.base_model),
                tokenizer=self.config.tokenizer_path or self.config.base_model,
                backend="hf",
                pretrain_mode="continue",
                params=overrides.pop("model_params", 7_000_000_000),
            ),
            method=RecipeMethodConfig(
                type=method,
            ...
            data=RecipeDataConfig(
                sources=[{"type": "local", "path": normalized_path if report else data_path, "format": "jsonl"}],
                output_dir=os.path.join(output_dir, "processed_data"),
        ...
        if save:
            os.makedirs(output_dir, exist_ok=True)
            payload["recipe_path"] = recipe.save(os.path.join(output_dir, f"{stage}_recipe.yaml"))
            payload["orchestrator_config_path"] = self._save_json(os.path.join(output_dir, "orchestrator_config.json"), compiled)
    ...
    def create_preflight_plan(
        ...
        **overrides,
        ...
        payload["training_estimate"] = self._estimate_from_orchestrator_config(payload["orchestrator_config"], stage=payload["stage"])
        if save:
            os.makedirs(output_dir, exist_ok=True)
            recipe = TrainingRecipe.from_dict(payload["recipe"])
            payload["recipe_path"] = recipe.save(os.path.join(output_dir, f"{payload['stage']}_preflight_recipe.yaml"))
            payload["orchestrator_config_path"] = self._save_json(
                os.path.join(output_dir, "orchestrator_config.json"),
                payload["orchestrator_config"],
            )
            payload["plan_path"] = self._save_json(os.path.join(output_dir, "preflight_plan.json"), payload)
        return payload

    def _estimate_from_orchestrator_config(self, config: Dict, stage: str) -> Dict:
    ...
    def create_mopd_plan(
        ...
        **overrides,
        ...
        When chain_sft=True, the recipe stages are mopd -> sft -> eval and SFT
        ...
        recipe = TrainingRecipe(
            ...
            method=RecipeMethodConfig(
                type=overrides.pop("method", "lora"),
                lora_r=overrides.pop("lora_r", 16),
                lora_alpha=overrides.pop("lora_alpha", 32),
                lora_dropout=overrides.pop("lora_dropout", 0.05),
            ),
        ...
        if save:
            os.makedirs(output_dir, exist_ok=True)
            payload["recipe_path"] = recipe.save(os.path.join(output_dir, "mopd_recipe.yaml"))
            payload["orchestrator_config_path"] = self._save_json(os.path.join(output_dir, "orchestrator_config.json"), compiled)
        ...
        **overrides,
        ...
        recipe = TrainingRecipe(
            ...
            vla=RecipeVLAConfig(
                ...
                action_bins=action_space.bins,
                action_min_value=action_space.min_value,
                action_max_value=action_space.max_value,
        ...
        if save:
            os.makedirs(output_dir, exist_ok=True)
            payload["recipe_path"] = recipe.save(os.path.join(output_dir, "vla_sft_recipe.yaml"))
            payload["orchestrator_config_path"] = self._save_json(os.path.join(output_dir, "orchestrator_config.json"), compiled)
    ...
    def create_pretrain_experiments(
        ...
        **overrides,
        ...
        from .PretrainExperimentRunner import PretrainExperimentConfig, PretrainExperimentRunner
