...
用法:
    ...
    # 或从命令行
    # python -m saddlellm.runtime.Deploy serve --model ./my_model --port 8000
"""
...
class Deploy:
    ...
    def _export_hf(model, tokenizer, output_dir, quantize, bits):
        ...
        if quantize:
            try:
                from ..compression.quantizer import ModelQuantizer

                q = ModelQuantizer(model=model, tokenizer=tokenizer)
                model = q.quantize(bits=bits)
    ...
    def _export_gguf(model, tokenizer, output_dir):
        ...
        try:
            ...
            logger.info(
                "已保存 HuggingFace 格式, 可用 llama.cpp 的 convert.py 手动转换"
            )
    ...
    def serve(
        ...
        async def generate(req: GenerateRequest):
            ...
            return GenerateResponse(
                text=text, tokens_generated=len(outputs[0]) - prompt_len
            )
...
if __name__ == "__main__":
    ...
    export_parser.add_argument(
        "--format", "-f", default="hf", choices=["hf", "onnx", "gguf"]
    )
    ...
    if args.command == "serve":
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.model)
        Deploy.export(
            model, tokenizer, args.output, format=args.format, quantize=args.quantize
        )
