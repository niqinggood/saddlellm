...
class ModelExperimentPlanner:
    ...
    def build_bundle(self) -> ModelExperimentBundle:
        from ..factory.FactoryBackendPlanner import FactoryBackendPlanner
        ...
        for idx, bp in enumerate(self.blueprints):
            ...
            run_config = self._config_for_blueprint(bp, run_dir)
            ...
            run_config["distributed"].update(FactoryBackendPlanner.to_training_orchestrator_distributed(backend_plan))
            run_config["backend_plan"] = backend_plan.to_dict()

            run_config_path = self._save_json(os.path.join(run_dir, "config.json"), run_config)
    ...
    def _config_for_blueprint(self, bp, run_dir: str) -> Dict:
        stages = ["pretrain", "eval"] if self.config.data_sources else ["eval"]
        config = {
            ...
            "eval": {"enabled": True, "max_samples": 100},
            "logging": {"output_dir": run_dir, "backend": "local"},
        }
        if not self.config.data_sources:
            config["notes"] = [
                "No data_sources supplied; architecture run is planning only."
            ]
        return config
    ...
    def _build_summary(self, runs: List[ModelExperimentRun]) -> Dict:
        from ..models.OperatorBackends import list_attention_backends
