...
import hashlib
import re
from typing import List, Optional, Dict, Iterator, Literal, Union
...
class DataPipeline:
    ...
    def _validate_config(self):
        ...
        if self.config.dedup_method not in {"simhash", "minhash", "exact", "none"}:
            raise ValueError("dedup_method must be one of: simhash, minhash, exact, none")
        if self.config.source_weights and len(self.config.source_weights) != len(self.config.sources):
    ...
    def collect(self) -> "DataPipeline":
        ...
        for source in self.config.sources:
            ...
            if src_type == "huggingface":
                ds = load_dataset(src_path, src_name, split=src_split, streaming=streaming,
                ...
                if not files:
                    raise FileNotFoundError(f"Local data source not found or empty: {path}")

                fmt_f = self._detect_format(files[0]) if fmt == "auto" else fmt
                ds = load_dataset(fmt_f, data_files=files, split="train", streaming=streaming,
                                      cache_dir=self.config.cache_dir)
                if text_col != "text":
                ...
                ds = ld("wikitext", "wikitext-103-raw-v1", split=src_split, streaming=streaming)
                all_splits.append(ds)
                ...
                ds = ld("wikipedia", f"20240301.{lang}", split=src_split, streaming=streaming)
                all_splits.append(ds)

            elif src_type == "c4":
                from datasets import load_dataset as ld
                lang = source.get("lang", "en")
                ds = ld("c4", f"{lang}", split=src_split, streaming=streaming)
                all_splits.append(ds)
    ...
    def clean(self) -> "DataPipeline":
        ...
        cleaner = None
        try:
            from .TextProcess import TextCleaner
            cleaner = TextCleaner({
            ...
            logger.warning("TextCleaner unavailable, using lightweight cleaner: %s", exc)
        ...
        def _clean(example):
            ...
            return {"text": text, "valid": bool(text) and len(text) >= self.config.min_text_length}
    ...
    def deduplicate(self, method: Optional[str] = None, threshold: Optional[float] = None) -> "DataPipeline":
    ...
    def _dedup_simhash(self, threshold):
        ...
        self._dataset = self._dataset.filter(
            lambda x: not _is_dup(x.get("text", ""))
        )
    ...
    def _dedup_exact(self):
        ...
        def _is_dup(text):
            key = hashlib.sha1(text.strip().lower()[:2000].encode("utf-8", errors="ignore")).hexdigest()
            if key in seen:
                return True
            seen.add(key)
            return False
    ...
    def filter_quality(self, min_length: Optional[int] = None, lang: Optional[str] = None) -> "DataPipeline":
        """
        质量过滤:
        - 最小文本长度
        - 语言检测
        ...
        lang_filter = lang or self.config.lang_filter
        ...
        if self.config.quality_min_score and self.config.quality_min_score > 0:
            try:
                ...
                quality_classifier = DataQualityClassifier(lang=lang_filter or "auto",
                                                           min_score=self.config.quality_min_score)
            except Exception as exc:
                logger.warning("DataQualityClassifier unavailable, using heuristic filter only: %s", exc)
        ...
        def _filter(example):
            ...
            # 标点符号比例
            punct_ratio = sum(1 for c in text if c in "，。！？；：、""''（）《》【】…—·,.!?;:()[]{}")
            ...
            if self.config.filter_urls:
                import re
                if re.search(r"https?://\S+", text) and len(re.findall(r"https?://\S+", text)) > 5:
                    return False
        ...
        def _pack_generator():
            for example in dataset:
                ...
                while len(buffer) >= max_len:
                    yield {"input_ids": buffer[:max_len], "labels": buffer[:max_len],
            ...
            if len(buffer) >= 10:
                ...
                yield {"input_ids": packed, "labels": labels,
        ...
        if hasattr(self._dataset, "save_to_disk"):
            ...
            for i, example in enumerate(self._dataset):
                ...
                if len(batch) >= batch_size:
                    pq.write_table(pa.Table.from_pylist(batch),
    ...
    def split(self, ratios: Optional[List[float]] = None, seed: Optional[int] = None):
        """划分训练/验证集"""
        ratios = ratios or self.config.split_ratios
        seed = seed or self.config.shuffle_seed
        ...
        split_dataset = self._dataset.train_test_split(
            test_size=ratios[1], seed=seed, shuffle=True
        )
...
class DataMixer:
    ...
    PRESETS = {
        "balanced_zh": [
            ("中文网页", 0.35), ("中文百科", 0.20), ("代码", 0.20),
            ("英文百科", 0.10), ("书籍", 0.10), ("对话", 0.05),
        ],
        "deepseek_style": [
            ("代码", 0.40), ("中英百科", 0.25), ("学术论文", 0.10),
            ("书籍", 0.10), ("通用网页", 0.10), ("对话", 0.05),
        ],
        "code_heavy": [
            ("代码", 0.50), ("技术文档", 0.20), ("百科", 0.15),
            ("通用文本", 0.15),
        ],
        "chinese_focused": [
            ("中文百科", 0.25), ("中文网页", 0.25), ("中文书籍", 0.15),
            ("中文对话", 0.10), ("代码", 0.15), ("英文百科", 0.10),
        ],
    }

    def __init__(self, configs: List[DataMixConfig],
    ...
    @classmethod
    def from_preset(cls, preset_name: str, source_paths: Dict[str, List[Dict]]) -> "DataMixer":
        """从预设配比创建 DataMixer"""
        ...
        for name, weight in cls.PRESETS[preset_name]:
            ...
            configs.append(DataMixConfig(
                name=name, weight=weight, sources=sources,
    ...
    def mix_and_process(
        ...
        # 加权交错混合
        logger.info(f"混合 {len(all_datasets)} 个数据源, 权重: "
