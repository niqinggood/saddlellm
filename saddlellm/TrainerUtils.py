...
""")
    ...
    return EnvCheckResult(
        python_version=py_ver, cuda_available=cuda_available,
        cuda_version=cuda_ver, gpu_count=gpu_count, gpu_name=gpu_name,
        gpu_memory_gb=gpu_mem, gpu_total_memoryory_gb=gpu_total,
        ram_total_gb=ram_total, ram_available_gb=ram_avail,
        disk_free_gb=disk_free, pytorch_version=pt_ver,
        transformers_version=transformers_ver, packages_ok=packages_ok,
        issues=issues, recommendations=recommendations, ready=ready,
    )
...
class TrainingDiagnostics:
    ...
    def check(self, step: int, loss: float, grad_norm: float = None,
              lr: float = None, mfu: float = None) -> Optional[str]:
        """
        检查训练状态, 返回诊断信息 (如果有问题)。
        ...
        if step >= 100 and len(self._loss_history) >= 20:
            ...
            older = self._loss_history[-40:-20] if len(self._loss_history) >= 40 else self._loss_history[:20]
        ...
        if len(self._loss_history) >= 10:
            avg = sum(self._loss_history[-10:-1]) / max(1, len(self._loss_history[-10:-1]))
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "loss_trend": "decreasing" if len(self._loss_history) >= 20 and self._loss_history[-1] < self._loss_history[0] else "flat",
        }
    ...
    def generate(
        ...
        report = {
            "model": {
                ...
                "total_params_human": f"{total_params/1e9:.2f}B" if total_params >= 1e9 else f"{total_params/1e6:.0f}M",
                "trainable_params": trainable_params,
                "architecture": type(model).__name__,
            },
            "training": training_config or {},
            "efficiency": mfu_summary or {},
            "evaluation": [{"task": r.task, "score": r.score, "metric": r.metric}
                           for r in eval_results] if eval_results else [],
        }
        ...
        if eval_results:
            ...
            if avg_score > 0.7: quality = "A"
            elif avg_score > 0.5: quality = "B"
        ...
        if mfu_summary:
            ...
            if mfu < 25:
                improvements.append("增大 batch_size 或减少 gradient_accumulation 提升 MFU")
        if total_params < 1e9:
...
def auto_train_best_small_model(
    ...
    config = DensePretrainConfig(
        max_steps=steps, learning_rate=lr, max_seq_length=seq_len,
        output_dir=output_dir, checkpoint_dir=os.path.join(output_dir, "ckpt"),
    ...
    if os.path.exists(ckpt_dir):
        ...
        if len(ckpts) >= 3:
            from .AdvancedTechniques import ModelSoup
    ...
    if use_api_teacher and env.cuda_available:
        from .RapidDistill import RapidDistill
        RapidDistill.run(model, tokenizer, use_teachers=api_teachers or ["gpt4o-mini"],
                         total_examples=200, output_dir=output_dir)
        print("✅ API 蒸馏完成")
    ...
    # 11. 报告卡
    TrainingReportCard.generate(model, training_config={"max_steps": steps},
