...
class DistributedTrainingConfig:
    strategy: Literal["single", "ddp", "fsdp", "deepspeed_zero2", "deepspeed_zero3"] = "single"
    num_gpus: int = 1
    num_nodes: int = 1
...
class TrainingConfig:
    ...
    multimodal: MultimodalTrainingConfig = field(default_factory=MultimodalTrainingConfig)
    media_cache: MediaCacheStageConfig = field(default_factory=MediaCacheStageConfig)
    image_generation: ImageGenerationTrainingConfig = field(default_factory=ImageGenerationTrainingConfig)
    music_generation: MusicGenerationTrainingConfig = field(default_factory=MusicGenerationTrainingConfig)
    video_generation: VideoGenerationTrainingConfig = field(default_factory=VideoGenerationTrainingConfig)
...
class TrainingOrchestrator:
    ...
    def _validate_config(self):
        valid_stages = {"tokenizer", "pretrain", "sft", "preference", "rlhf", "mopd", "mllm_sft", "vision_alignment", "media_cache", "image_generation", "music_generation", "video_generation", "world_model", "vla_sft", "eval", "export", "operator"}
        unknown = [stage for stage in self.config.stages if stage not in valid_stages]
        if unknown:
            raise ValueError(f"Unknown training stages: {unknown}. Available stages: {sorted(valid_stages)}")
        ...
        if self.config.sft.enabled and not self.config.sft.data_path and not sft_can_use_mopd:
        ...
        if self.config.image_generation.enabled and not self.config.image_generation.data_path and not self._has_upstream_media_cache("image", "image_generation"):
            raise ValueError(
                "image_generation requires data_path or an upstream image media_cache job"
            )
        if self.config.music_generation.enabled and not self.config.music_generation.data_path and not self._has_upstream_media_cache("audio", "music_generation"):
            raise ValueError(
                "music_generation requires data_path or an upstream audio media_cache job"
            )
        if self.config.video_generation.enabled and not self.config.video_generation.data_path and not self._has_upstream_media_cache("video", "video_generation"):
            raise ValueError(
                "video_generation requires data_path or an upstream video media_cache job"
            )
        if self.config.world_model.enabled:
            if not self.config.world_model.config_path and not self.config.world_model.data.get(
                "train_path"
            ):
                raise ValueError(
        ...
        if self.config.preference.enabled:
            if self.config.preference.method not in {"dpo", "orpo", "kto"}:
                raise ValueError(f"Unsupported preference.method: {self.config.preference.method}")
            preference_actual_batch_size = (
                self.config.preference.per_device_batch_size * max(1, self.config.distributed.num_gpus)
            )
            if self.config.preference.method == "kto" and preference_actual_batch_size <= 1:
                raise ValueError(
        ...
        if self.config.distributed.bf16 and self.config.distributed.fp16:
            raise ValueError("distributed.bf16 and distributed.fp16 cannot both be enabled")
        ...
        if self.config.distributed.num_gpus < 1 or self.config.distributed.num_nodes < 1:
            raise ValueError("distributed.num_gpus and distributed.num_nodes must be >= 1")
        ...
        if self.config.distributed.strategy != "single":
            unsupported_distributed_stages = set(self.config.stages) - {"pretrain"}
            if unsupported_distributed_stages:
                raise ValueError(
                    "Distributed execution currently supports the pretrain stage only; "
                    "run downstream stages separately in single-process mode: "
        ...
        if "operator" in self.config.stages:
            if not self.config.operator.enabled or not self.config.operator.label:
                raise ValueError("operator stage requires operator.enabled=true and operator.label")
        ...
        if self.config.model_backend == "saddle":
            if self.config.pretrain_mode == "continue" and self.config.training.resume_from_checkpoint:
                raise ValueError(
            ...
            if self.config.training.resume_from_checkpoint and "pretrain" not in self.config.stages:
                raise ValueError(
            ...
            from .ModelAdapter import get_model_adapter

            adapter = get_model_adapter("saddle")
            unsupported_saddle_stages = set(self.config.stages) - set(
                adapter.capabilities.stages
            )
            if unsupported_saddle_stages:
                raise ValueError(
                    "Native Saddle models do not support these configured stages: "
                    + ", ".join(sorted(unsupported_saddle_stages))
                )
            if "sft" in self.config.stages:
                adapter.validate_stage(
                    "sft",
                    use_lora=self.config.sft.use_lora,
                    use_qlora=self.config.sft.use_qlora,
                )
            if "preference" in self.config.stages:
                adapter.validate_stage(
                    "preference",
    ...
    def _has_upstream_media_cache(self, modality: str, consumer: str) -> bool:
        ...
        return any(
            isinstance(job, dict)
            and str(job.get("modality", "")).lower() == modality
    ...
    def _validate_architecture_support(self):
        ...
        from .ModelRegistry import MODEL_SPECS
        ...
        if "pretrain" in self.config.stages:
            if self.config.pretrain_mode == "scratch":
                ...
                support = ArchitectureRegistry.detect_from_model_name(self.config.model_name_or_path)
                if not support.supports("continue_pretrain"):
                    raise ValueError(f"Model architecture does not support continued pretraining: {support.name} ({support.notes})")
    ...
    def _resolve_model_blueprint(self, spec=None):
        from .ModelBlueprint import ModelBlueprint
        from .ModelRegistry import MODEL_SPECS
    ...
    def _resolve_exact_resume_checkpoint(self) -> Optional[str]:
        ...
        if resume is True:
            ...
            checkpoint_path = get_last_checkpoint(checkpoint_root) if os.path.isdir(checkpoint_root) else None
            ...
            raise TypeError("training.resume_from_checkpoint must be false, true, or a checkpoint path")
        ...
        missing = sorted(name for name in required if not os.path.isfile(os.path.join(checkpoint_path, name)))
        ...
        if expected_world_size == 1:
            ...
            missing.extend(
                ...
                if not os.path.isfile(os.path.join(checkpoint_path, f"rng_state_{rank}.pth"))
            )
        ...
        if os.path.isfile(manifest_path):
            ...
            if saved_world_size is not None and int(saved_world_size) != expected_world_size:
                raise ValueError(
    ...
    @classmethod
    def from_yaml(cls, path: str) -> "TrainingOrchestrator":
    ...
    @classmethod
    def from_dict(cls, data: Dict) -> "TrainingOrchestrator":
    ...
    def run(self):
        ...
        elapsed = time.time() - start_time
        logger.info(f"=== Training pipeline completed in {elapsed/3600:.1f} hours ===")
        try:
            self._print_summary(elapsed_seconds=elapsed)
        finally:
            self._close_logging_handlers()
    ...
    def run_stage(self, stage: str):
        ...
        if (
            ...
            raise ValueError(
                f"Stage {stage!r} is not distributed-safe yet; run it in a separate "
                "single-process pipeline after distributed pretraining"
            )
        logger.info(f"{'='*60}")
        ...
        if stage == "tokenizer":
            self._run_tokenizer_stage()
        elif stage == "pretrain":
            self._run_pretrain_stage()
        elif stage == "sft":
            self._run_sft_stage()
        elif stage == "preference":
            self._run_preference_stage()
        elif stage == "rlhf":
            self._run_rlhf_stage()
        elif stage == "mopd":
            self._run_mopd_stage()
        elif stage == "vla_sft":
            self._run_vla_stage()
        elif stage in {"mllm_sft", "vision_alignment"}:
            self._run_multimodal_stage()
        elif stage == "media_cache":
            self._run_media_cache_stage()
        elif stage == "image_generation":
            self._run_image_generation_stage()
        elif stage == "music_generation":
            self._run_music_generation_stage()
        elif stage == "video_generation":
            self._run_video_generation_stage()
        elif stage == "world_model":
            self._run_world_model_stage()
        elif stage == "eval":
            self._run_eval_stage()
        elif stage == "export":
            self._run_export_stage()
        elif stage == "operator":
            self._run_operator_stage()
        else:
            logger.warning(f"Unknown stage skipped: {stage}")
    ...
    def _run_operator_stage(self):
        ...
        from saddle_ml.agent import RESULT_SCHEMA_VERSION, SUPPORTED_AGENT_OPERATORS, run_agent_operator
        ...
        try:
            if stage.label in SUPPORTED_AGENT_OPERATORS:
                ...
                from .RealWorldOperators import run_large_model_operator
            ...
            try:
                ...
                result["warnings"].append("Could not persist Agent error artifact: %s" % artifact_exc)
        ...
        if str(result.get("status", "")).lower() in {"error", "failed", "failure"}:
            raise RuntimeError(result.get("message") or "%s operator failed" % stage.label)
        logger.info("Visual operator %s completed: %s", stage.label, result.get("message", ""))

    def _run_tokenizer_stage(self):
        """Stage 1: train tokenizer."""
        from .TokenizerTrainer import TokenizerTrainer, TokenizerConfig as TokConfig

        tok_config = self.config.tokenizer_training
        if tok_config is None:
            logger.info("No tokenizer_training config found; loading an existing tokenizer")
    ...
    def _run_pretrain_stage(self):
        ...
        if spec is None and not (
            ...
            and (self.config.model_blueprint or exact_resume_path or self.config.pretrain_mode == "continue")
        ):
            raise KeyError(f"Unknown model config: {self.config.model_config}. Available: {list(MODEL_SPECS.keys())}")

        if spec is not None:
            logger.info(f"Model spec: {spec.name} ({spec.human_params()} params, {spec.human_tokens()} target tokens)")
        ...
        if exact_resume_path:
            ...
            continue_dtype = (
                torch.bfloat16 if self.config.distributed.bf16
                else torch.float16 if self.config.distributed.fp16
                else torch.float32
            )
            ...
            logger.info(
                ...
                spec.name if spec is not None else self.config.model_blueprint.get("name"),
            )
            if self.config.model_backend == "saddle":
        ...
        if exact_resume_path:
            ...
            self._tokenizer = load_tokenizer_compatible(exact_resume_path, trust_remote_code=True)
        else:
            self._load_tokenizer()
        ...
        grad_accum = self.config.distributed.gradient_accumulation_steps or max(
            1, self.config.training.global_batch_size // (
                self.config.training.per_device_batch_size * max(1, self.config.distributed.num_gpus)
            )
        )
        dist_cfg = DistCfg(
            strategy=self.config.distributed.strategy,
        ...
        for duplicate_key in ("gradient_accumulation_steps", "bf16", "fp16", "dataloader_num_workers"):
        ...
        training_args = TrainingArguments(
            ...
            report_to=[self.config.logging.logging_backend] if self.config.logging.logging_backend != "local" else ["none"],
        ...
        if self.config.training.stability_monitor:
            from .PretrainStability import PretrainStabilityCallback, StabilityConfig

            stability_dir = self.config.training.stability_output_dir or os.path.join(self._output_dir, "stability")
            callbacks.append(PretrainStabilityCallback(StabilityConfig(
                enabled=True,
        ...
        if self.config.model_backend == "saddle":
            from .ModelMetrics import ModelMetricsCallback
        ...
        logger.info(f"Starting pretraining: {self.config.training.max_steps} steps, "
                    f"batch_size={self.config.training.per_device_batch_size}, "
                    f"grad_accum={grad_accum}")

        trainer.train(resume_from_checkpoint=self.config.training.resume_from_checkpoint)
        ...
        self._stage_results["pretrain"] = {
            ...
            "continuation": "weights_only" if self.config.pretrain_mode == "continue" else "scratch_or_exact_resume",
            "resume_from_checkpoint": self.config.training.resume_from_checkpoint or None,
            "total_params": total_params,
            "estimated_params": spec.estimated_params if spec is not None else total_params,
        }
        distributed_barrier()
    ...
    def _inspect_post_training_data(self, path: str, task: str) -> Dict:
        ...
        if not report.ready:
            self._close_logging_handlers()
            raise ValueError(f"{task} data inspection failed: {'; '.join(report.errors)}")
    ...
    def _run_sft_stage(self):
        ...
        model_path = self._stage_results.get("pretrain", {}).get("model_path") or self.config.model_name_or_path
        if not model_path:
            raise ValueError("SFT requires model.name_or_path or an upstream pretrain output")

        sft_cfg = self.config.sft
        dataset_path = sft_cfg.data_path or self._stage_results.get("mopd", {}).get("data_path")
        if not dataset_path:
            raise ValueError("SFT requires sft.data_path or a completed upstream MOPD stage with data_path")
        ...
        plan = self._write_stage_plan("sft", {
            "stage": "sft",
            "status": "planned",
        ...
        try:
            if self.config.model_backend == "saddle":
                ...
                result = train_native_sft(NativeSFTConfig(
                    model_path=model_path,
            ...
            logger.info("Use --debug for a full traceback from PeftSFTTrainer.train_model()")
    ...
    def _run_preference_stage(self):
        ...
        logger.info(f"Starting preference training stage ({self.config.preference.method})")
        ...
        if not model_path:
            raise ValueError("Preference training requires model.name_or_path or an upstream sft/pretrain output")

        pref_cfg = self.config.preference
        output_path = os.path.join(self._output_dir, "preference_checkpoints")
        data_inspection = self._inspect_post_training_data(pref_cfg.data_path, task=pref_cfg.method)
        plan = self._write_stage_plan("preference", {
            "stage": "preference",
            "status": "planned",
            "model_path": model_path,
        ...
        if self.config.model_backend == "saddle":
            ...
            result = train_native_dpo(NativeDPOConfig(
                model_path=model_path,
                ...
                report_to=pref_cfg.report_to if pref_cfg.report_to != "local" else "none",
                seed=self.config.seed,
            ))
        else:
            from .Preference import PreferenceTrainConfig, train_preference

            result = train_preference(PreferenceTrainConfig(
                model_path=model_path,
                dataset_path=pref_cfg.data_path,
                output_path=output_path,
    ...
    def _run_rlhf_stage(self):
        ...
        model_path = self._stage_results.get("preference", {}).get("model_path") or \
                     self._stage_results.get("sft", {}).get("model_path") or \
                     self._stage_results.get("pretrain", {}).get("model_path") or \
                     self.config.model_name_or_path

        if not model_path:
            raise ValueError("RLHF requires model_name_or_path or an upstream trained model.")
        ...
        plan = self._write_stage_plan("rlhf", {
            "stage": "rlhf",
            "status": "planned",
    ...
    def _run_multimodal_stage(self):
        ...
        if cfg.data_path and os.path.exists(cfg.data_path):
            ...
            plan["warnings"] = ["No multimodal data_path found; wrote planning artifacts only."]
    ...
    def _run_image_generation_stage(self):
        ...
        if (
            ...
            return
        from .LatentFlowTrainer import CachedLatentDataset
        ...
        if not os.path.isabs(output_path):
            output_path = os.path.join(self._output_dir, output_path)
        plan = self._write_stage_plan("image_generation", {
        ...
        if self.config.training.dry_run or self.config.training.preflight_only:
            ...
            return
        from .LatentFlowTrainer import LatentFlowTrainingConfig, train_latent_flow
    ...
    def _run_music_generation_stage(self):
        ...
        if (
            ...
            return
        from .MusicCodeModel import MusicCodeConfig
        from .MusicCodeTrainer import CachedMusicCodeDataset
        ...
        if cfg.num_codebooks is not None and int(cfg.num_codebooks) != inferred.num_codebooks:
            raise ValueError(
                f"music_generation.num_codebooks={cfg.num_codebooks} does not match "
                f"cached data value {inferred.num_codebooks}"
            )
        if cfg.condition_dim is not None and int(cfg.condition_dim) != inferred.condition_dim:
            raise ValueError(
        ...
        if not os.path.isabs(output_path):
            output_path = os.path.join(self._output_dir, output_path)
        plan = self._write_stage_plan("music_generation", {
        ...
        if self.config.training.dry_run or self.config.training.preflight_only:
            ...
            return
        from .MusicCodeTrainer import MusicCodeTrainingConfig, train_music_code
    ...
    def _run_video_generation_stage(self):
        ...
        if (
            ...
            return
        from .VideoLatentFlowTrainer import CachedVideoLatentDataset
        ...
        for name, configured in dimensions.items():
            if configured is not None and int(configured) != getattr(model_config, name):
                raise ValueError(
        ...
        if not os.path.isabs(output_path):
            output_path = os.path.join(self._output_dir, output_path)
        plan = self._write_stage_plan("video_generation", {
        ...
        if self.config.training.dry_run or self.config.training.preflight_only:
            ...
            return
        from .VideoLatentFlowTrainer import (
    ...
    def _run_world_model_stage(self):
        ...
        plan = self._write_stage_plan("world_model", {
            "stage": "world_model",
    ...
    def _run_vla_stage(self):
        ...
        cfg = self.config.vla
        output_dir = os.path.join(self._output_dir, "vla") if cfg.output_dir == "./vla" else cfg.output_dir
        os.makedirs(output_dir, exist_ok=True)

        from .VLA import VLAActionSpace, VLADataAdapter, VLATrainingPlanner
        ...
        if cfg.data_path and os.path.exists(cfg.data_path):
            ...
            plan["status"] = "planned_no_data"
            plan["warnings"] = ["No VLA data_path found; wrote planning artifacts only."]

        train_result = None
        if cfg.train and plan.get("normalized_data_path"):
            from .VLATrainer import VLASFTConfig, train_vla_sft

            train_result = train_vla_sft(VLASFTConfig(
                model_path=self.config.model_name_or_path or self.config.model_config,
    ...
    def _run_mopd_stage(self):
        ...
        output_dir = os.path.join(self._output_dir, "mopd") if cfg.output_dir == "./mopd" else cfg.output_dir
        os.makedirs(output_dir, exist_ok=True)
        prompts = self._load_mopd_prompts(cfg.prompts_path)
        if not cfg.dry_run and prompts:
            self._ensure_policy_model_loaded()

        from .OnPolicyDistillation import MOPDConfig, MultiTeacherOnPolicyDistiller, OnPolicyTeacherSpec
        ...
        if not teacher_specs:
            teacher_specs = [OnPolicyTeacherSpec(name="placeholder_teacher", domain="general", weight=1.0)]
        ...
        if cfg.dry_run or not prompts:
            ...
            if not prompts:
                plan["warnings"] = ["No prompts_path found; wrote planning artifacts only."]
        else:
            records = distiller.collect(prompts)
            data_path = distiller.save_jsonl(os.path.join(output_dir, "mopd_on_policy.jsonl"), records)
    ...
    def _ensure_policy_model_loaded(self):
        ...
        if not model_path:
            raise ValueError("MOPD collection requires a loaded policy model or model.name_or_path")
        self._load_tokenizer()
    ...
    def _build_mopd_teacher(self, raw: Dict):
        ...
        if provider == "anthropic":
            return TeacherInterface.from_anthropic(model or "claude-sonnet-4-6", api_key=api_key)
        if provider == "deepseek":
            return TeacherInterface.from_deepseek(model or "deepseek-chat", api_key=api_key)
        if provider == "endpoint":
            return TeacherInterface(model_type="endpoint", model_name=model, endpoint_url=raw.get("endpoint_url"))
            ...
            with open(path, "r", encoding="utf-8") as f:
                for line in f:
                    if not line.strip():
                        continue
                    row = json.loads(line)
                    prompt = row.get("prompt") or row.get("instruction") or row.get("question") or row.get("text")
                    if prompt:
                        prompts.append(str(prompt))
        elif ext == ".json":
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            rows = data if isinstance(data, list) else data.get("data", data.get("prompts", []))
            for row in rows:
    ...
    def _run_eval_stage(self):
        ...
        if not model_path:
            raise ValueError("Evaluation requires model.name_or_path or an upstream trained model.")
        ...
        try:
            ...
            logger.info(f"Evaluation results: {json.dumps(results, ensure_ascii=False)}")
        except Exception as e:
        ...
        gate_path = gate_config.output_path or os.path.join(self._output_dir, "release_gate.json")
        gate.save(gate_path)
        ...
        if gate_config.enabled and not gate.accepted:
            message = "Evaluation release gate rejected the model: " + "; ".join(gate.failures)
    ...
    def _run_export_stage(self):
        ...
        if not export_config.enabled:
            self._stage_results["export"] = {"status": "skipped", "reason": "export.enabled is false"}
            return
        ...
        if not model_path:
            raise ValueError("Export requires export.model_path, model.name_or_path, or an upstream trained model.")

        eval_result = self._stage_results.get("eval", {})
        gate_payload = eval_result.get("release_gate") if isinstance(eval_result, dict) else None
        ...
        output_dir = export_config.output_dir or os.path.join(self._output_dir, "release")
        if self.config.training.dry_run or self.config.training.preflight_only:
    ...
    def _load_tokenizer(self):
        ...
        if tok_path:
            self._tokenizer = load_tokenizer_compatible(tok_path, trust_remote_code=True)
        elif self.config.tokenizer_training and os.path.exists(self.config.tokenizer_training.output_dir):
            ...
            logger.warning("No tokenizer path found; falling back to the gpt2 tokenizer")
    ...
    def _parse_config(raw: Dict) -> TrainingConfig:
        ...
        nested_blueprint = model_raw.get("blueprint") if isinstance(model_raw, dict) else None
        if (
        ...
        for s in raw.get("data", {}).get("sources", []):
            data_sources.append(DataSourceConfig(
                ...
                path=s.get("path", s.get("paths", [""])[0] if isinstance(s.get("paths"), list) else s.get("paths", "")),
                name=s.get("name"),
        ...
        if tok_raw:
            tok_config = TokenizerTrainingConfig(
                ...
                eval_domain_terms=tok_raw.get("eval_domain_terms", tok_raw.get("domain_terms", [])),
            )
        ...
        if strategy not in {"single", "ddp", "fsdp", "deepspeed_zero2", "deepspeed_zero3"}:
        ...
        cfg = TrainingConfig(
            ...
            tokenizer_name_or_path=model_raw.get("tokenizer",
                                                           raw.get("tokenizer_name_or_path")),
            model_backend=model_backend,
            model_blueprint=model_blueprint,
            pretrain_mode=raw.get("pretrain_mode", "scratch"),
            stages=raw.get("stages", ["tokenizer", "pretrain", "eval"]),
            ...
            training=TrainingHyperparams(
                ...
                stability_spike_threshold=train_raw.get("stability_spike_threshold", 2.5),
                stability_stagnation_window=train_raw.get("stability_stagnation_window", 200),
            ...
            distributed=DistributedTrainingConfig(
                ...
                gradient_accumulation_steps=dist_raw.get("gradient_accumulation_steps", 1),
            ...
            sft=SFTConfig(
                ...
                lora_dropout=sft_lora_raw.get("dropout", sft_raw.get("lora_dropout", 0.05)),
                lora_target_modules=sft_lora_raw.get(
                    "target_modules",
                    sft_raw.get("lora_target_modules", ["q_proj", "k_proj", "v_proj", "o_proj"]),
                ),
                ...
                max_seq_length=sft_raw.get("max_seq_length", raw.get("data", {}).get("max_seq_length", 2048)),
                gradient_accumulation_steps=sft_raw.get("gradient_accumulation_steps", dist_raw.get("gradient_accumulation_steps", 4)),
                warmup_steps=sft_raw.get("warmup_steps", train_raw.get("warmup_steps", 100)),
                save_steps=sft_raw.get("save_steps", train_raw.get("save_every_steps", 200)),
                eval_steps=sft_raw.get("eval_steps", train_raw.get("eval_every_steps", 0)),
                logging_steps=sft_raw.get("logging_steps", train_raw.get("log_every_steps", 10)),
            ...
            rlhf=RLHFConfig(
                ...
                max_length=rlhf_raw.get("max_length", raw.get("data", {}).get("max_seq_length", 2048)),
                max_prompt_length=rlhf_raw.get("max_prompt_length", max(128, raw.get("data", {}).get("max_seq_length", 2048) // 2)),
                ...
                gradient_accumulation_steps=rlhf_raw.get("gradient_accumulation_steps", dist_raw.get("gradient_accumulation_steps", 4)),
                warmup_steps=rlhf_raw.get("warmup_steps", train_raw.get("warmup_steps", 100)),
                save_steps=rlhf_raw.get("save_steps", train_raw.get("save_every_steps", 200)),
            ),
            preference=PreferenceConfig(
                ...
                lora_dropout=pref_lora_raw.get("dropout", pref_raw.get("lora_dropout", 0.05)),
                gradient_accumulation_steps=pref_raw.get("gradient_accumulation_steps", dist_raw.get("gradient_accumulation_steps", 4)),
                warmup_steps=pref_raw.get("warmup_steps", train_raw.get("warmup_steps", 100)),
                save_steps=pref_raw.get("save_steps", train_raw.get("save_every_steps", 200)),
                logging_steps=pref_raw.get("logging_steps", train_raw.get("log_every_steps", 10)),
                validation_split=pref_raw.get("validation_split", 0.0),
                eval_steps=pref_raw.get("eval_steps", train_raw.get("eval_every_steps", 0)),
                ...
                report_to=pref_raw.get("report_to", raw.get("logging", {}).get("backend", "none")),
            ),
            multimodal=MultimodalTrainingConfig(
                ...
                image_token_strategy=multimodal_raw.get("image_token_strategy", "replace"),
                ...
                max_seq_length=multimodal_raw.get("max_seq_length", raw.get("data", {}).get("max_seq_length", 2048)),
            ),
            ...
            image_generation=ImageGenerationTrainingConfig(
                ...
                codec_name=image_generation_raw.get("codec_name", "external-image-codec"),
            ...
            music_generation=MusicGenerationTrainingConfig(
                ...
                output_dir=music_generation_raw.get(
                    "output_dir", "./music_generation"
                ),
            ...
            video_generation=VideoGenerationTrainingConfig(
                ...
                output_dir=video_generation_raw.get(
                    "output_dir", "./video_generation"
                ),
                ...
                temporal_patch_size=video_generation_raw.get(
            ...
            vla=VLATrainingConfig(
                ...
                action_dim=vla_raw.get("action_space", {}).get("action_dim", vla_raw.get("action_dim", 7)),
                action_type=vla_raw.get("action_space", {}).get("action_type", vla_raw.get("action_type", "continuous")),
                action_bins=vla_raw.get("action_space", {}).get("bins", vla_raw.get("action_bins", 256)),
                action_min_value=vla_raw.get("action_space", {}).get("min_value", vla_raw.get("action_min_value", -1.0)),
                action_max_value=vla_raw.get("action_space", {}).get("max_value", vla_raw.get("action_max_value", 1.0)),
                include_gripper=vla_raw.get("action_space", {}).get("include_gripper", vla_raw.get("include_gripper", True)),
                control_hz=vla_raw.get("action_space", {}).get("control_hz", vla_raw.get("control_hz", 10.0)),
                coordinate_frame=vla_raw.get("action_space", {}).get("coordinate_frame", vla_raw.get("coordinate_frame", "end_effector_delta")),
                ...
                gradient_accumulation_steps=vla_raw.get("gradient_accumulation_steps", 4),
                validation_split=vla_raw.get("validation_split", 0.0),
                max_seq_length=vla_raw.get("max_seq_length", raw.get("data", {}).get("max_seq_length", 2048)),
            ...
            operator=OperatorStageConfig(
                ...
                work_dir=operator_raw.get("work_dir", raw.get("logging", {}).get("output_dir", "./outputs")),
            ),
        )
        return cfg
