...
LIGHTWEIGHT_RECIPES = {
    "100m": {
        "quantization": {"method": "int8", "bits": 8, "group_size": 128, "desc": "int8 量化, 几乎无精度损失"},
        "pruning": {"method": "unstructured", "sparsity": 0.1, "desc": "10% 非结构化剪枝"},
        "kv_cache": {"bits": 8, "desc": "KV Cache int8"},
        "spec_decoding": {"draft_model": "gpt2-small-124m", "desc": "同模型草稿 (self-speculative)"},
        "estimated_size": "500MB → 300MB (int8)",
        "estimated_speedup": "1.3x",
    },
    "300m": {
        "quantization": {"method": "int4", "bits": 4, "group_size": 128, "desc": "int4 量化 (GPTQ/AWQ推荐)"},
        "pruning": {"method": "unstructured", "sparsity": 0.15, "desc": "15% 非结构化剪枝"},
        "kv_cache": {"bits": 8, "desc": "KV Cache int8"},
        "spec_decoding": {"draft_model": "gpt2-small-124m", "desc": "GPT-2 124M 草稿模型"},
        "estimated_size": "1.2GB → 400MB (int4)",
        "estimated_speedup": "1.8x",
    },
    "1b": {
        "quantization": {"method": "int4", "bits": 4, "group_size": 128, "desc": "int4 GPTQ 量化"},
        "pruning": {"method": "structured", "sparsity": 0.2, "desc": "20% 结构化剪枝 (2:4)"},
        "kv_cache": {"bits": 4, "desc": "KV Cache int4"},
        "spec_decoding": {"draft_model": "llama-tiny-150m", "desc": "150M 草稿 → 1B 审阅"},
        "estimated_size": "4GB → 1.2GB (int4+剪枝)",
        "estimated_speedup": "2.5x",
    },
    "3b": {
        "quantization": {"method": "int4", "bits": 4, "group_size": 64, "desc": "int4 GPTQ group64"},
    ...
    "7b": {
        "quantization": {"method": "int4", "bits": 4, "group_size": 64, "desc": "int4 AWQ 量化"},
        "pruning": {"method": "structured", "sparsity": 0.3, "desc": "30% 结构化剪枝 (配合微调恢复)"},
        "kv_cache": {"bits": 4, "desc": "KV Cache int4"},
...
class Lightweight:
    ...
    def recommend(model_size: str = "1b"):
        ...
        if recipe is None:
            print(f"无预设方案, 使用通用建议:")
            print(f"  量化: int8 (安全) 或 int4 (激进)")
            print(f"  剪枝: 10-20% 非结构化")
...
""")
    ...
    def optimize(
        ...
        if target == "auto":
            if total_params < 3e8: target = "300m"
            elif total_params < 1e9: target = "1b"
            elif total_params < 5e9: target = "3b"
            else: target = "7b"
        ...
        if apply_quantization:
            ...
            print(f"Step 1: {q_method} {q_bits}bit 量化...")
            try:
                from .ModelQuantizer import ModelQuantizer
                q = ModelQuantizer()
                q.quantize(model, bits=q_bits)
                print(f"  完成: {total_params*2/1024**3:.1f}GB → {total_params*q_bits/8/1024**3:.1f}GB")
            except ImportError:
                print("  量化需要 bitsandbytes/auto-gptq, 跳过")

        # Step 2: 剪枝
        if apply_pruning:
            sparsity = recipe["pruning"]["sparsity"]
            print(f"Step 2: {sparsity*100:.0f}% 剪枝...")
            try:
                from .ModelPruner import ModelPruner
                p = ModelPruner()
                p.prune(model, sparsity=sparsity)
        ...
        import json
        config = {
            "quantization": recipe["quantization"] if apply_quantization else None,
            "pruning": recipe["pruning"] if apply_pruning else None,
        ...
        final_size_gb = os.path.getsize(os.path.join(output_dir, "model.safetensors")) / 1024**3 if os.path.exists(os.path.join(output_dir, "model.safetensors")) else 0
        print(f"\n=== 轻量化完成! {output_dir} ===")
        if final_size_gb:
            print(f"  最终大小: {final_size_gb:.2f}GB")

    @staticmethod
    def quantize_only(model, tokenizer, output_dir: str = "./quantized_model",
                       bits: int = 4, method: str = "auto"):
        """只做量化。"""
        from .ModelQuantizer import ModelQuantizer
        q = ModelQuantizer()
        q.quantize(model, bits=bits, method=method)
        model.save_pretrained(output_dir)
        tokenizer.save_pretrained(output_dir)
    ...
    def speculative_generate(
        ...
        if draft_model is None:
            ...
            return Lightweight._standard_generate(model, tokenizer, prompt, max_new_tokens, temperature)
        ...
        while len(generated_ids) < max_new_tokens:
            # ---- 草稿阶段: 小模型快速生成候选 ----
            current_ids = torch.cat([input_ids, torch.tensor([generated_ids], device=device).T if generated_ids else input_ids[:, :0]], dim=1) if generated_ids else input_ids
            ...
            with torch.no_grad():
                ...
                draft_outputs = draft_model.generate(
                    ...
                    pad_token_id=draft_tokenizer.pad_token_id or draft_tokenizer.eos_token_id,
                    output_scores=False,
        ...
        response = full_text[len(tokenizer.decode(input_ids[0], skip_special_tokens=True)):]

        logger.info(f"投机解码: {total_accepted}/{total_drafted} tokens accepted ({acceptance_rate:.1%})")
        ...
        return {
            ...
            "estimated_speedup": round(1 + acceptance_rate * (num_draft_tokens - 1) * 0.5, 1),
        }
    ...
    def _standard_generate(model, tokenizer, prompt, max_tokens, temperature):
        ...
        return {"text": full[prompt_len:].strip(), "generated_tokens": len(outputs[0]) - inputs["input_ids"].shape[1]}
    ...
    def _configure_kv_cache(model, bits: int = 8):
        ...
        logger.info(f"  推理时使用: model.generate(..., cache_implementation='quantized')")
        logger.info(f"  或配置: model.config.cache_implementation = 'quantized'")
