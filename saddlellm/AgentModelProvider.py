...
def chat_completion(
    ...
    try:
        with _open_provider_request(request, timeout=int(provider.get("timeoutSeconds", 60))) as response:
        ...
        raise ModelProviderError("provider request failed: %s" % exc.reason) from exc
    try:
