...
class ModelSpec:
    name: str
    architecture: str                    # "llama" | "gpt2" | "gpt-neox" | "mixtral" | "deepseek" | "mamba"
    hidden_size: int
    ...
    def _estimate_params(self) -> int:
        ...
        return embed + (ffn_params + attn_params) * n + norm_params + final_norm + lm_head
    ...
    def _estimate_active_params(self) -> int:
        ...
        d = self.hidden_size; n = self.num_hidden_layers; v = self.vocab_size
        h_q = self.num_attention_heads
        kv = self.num_kv_heads if self.num_kv_heads > 0 else self.num_attention_heads
        head_dim = d // h_q
        ...
        if self.kv_lora_rank > 0:
            attn_per_layer = (d * self.kv_lora_rank + self.kv_lora_rank * (kv + h_q) * head_dim +
                              d * self.q_lora_rank + self.q_lora_rank * h_q * head_dim + h_q * head_dim * d)
        else:
            attn_per_layer = d * h_q * head_dim + d * kv * head_dim * 2 + h_q * head_dim * d
    ...
    def to_dict(self) -> dict:
        d = {
            ...
            "num_kv_heads": self.num_kv_heads, "intermediate_size": self.intermediate_size,
            "vocab_size": self.vocab_size, "max_position_embeddings": self.max_position_embeddings,
        ...
        if self.num_experts > 0:
            d.update({"num_experts": self.num_experts, "num_experts_per_tok": self.num_experts_per_tok})
        if self.kv_lora_rank > 0:
            d.update({"kv_lora_rank": self.kv_lora_rank, "q_lora_rank": self.q_lora_rank})
        return d
...
class ModelRegistry:
    ...
    @staticmethod
    def list_by_size(max_params: Optional[int] = None, min_params: Optional[int] = None) -> List[ModelSpec]:
        """按参数量筛选"""
    ...
    def architecture_support(name_or_spec) -> Dict:
        ...
        spec = ModelRegistry.get(name_or_spec) if isinstance(name_or_spec, str) else name_or_spec
    ...
    @staticmethod
    def create_model(spec: ModelSpec, vocab_size_override: Optional[int] = None) -> "PreTrainedModel":
        """
        ...
        if spec.architecture == "llama":
            config = LlamaConfig(
                ...
                num_key_value_heads=spec.num_kv_heads if spec.num_kv_heads > 0 else spec.num_attention_heads,
            ...
            return AutoModelForCausalLM.from_config(config, torch_dtype=_get_torch_dtype())
            ...
            config = MixtralConfig(
                ...
                expert_intermediate_size=spec.expert_intermediate_size or spec.intermediate_size,
            ...
            logger.warning(
                ...
                spec.kv_lora_rank, spec.q_lora_rank,
            )
            from transformers import MixtralConfig
            config = MixtralConfig(
                hidden_size=spec.hidden_size,
...
"""
    ...
    @staticmethod
    def recommend_for_gpu(gpu_memory_gb: int, strategy: str = "full_finetune") -> List[ModelSpec]:
        """根据 GPU 显存推荐可行的模型"""
        ...
        if strategy == "full_finetune":
            ...
            for spec in sorted(MODEL_SPECS.values(), key=lambda s: s.estimated_params):
                mem = ModelRegistry.estimate_memory(spec, batch_size=1, optimizer="adamw")
            ...
            for spec in sorted(MODEL_SPECS.values(), key=lambda s: s.estimated_params):
                mem = ModelRegistry.estimate_memory(spec, batch_size=1, optimizer="adamw")
                lora_mem = mem["model_gb"] + mem["optimizer_gb"] * 0.15 + mem["activations_gb"]
